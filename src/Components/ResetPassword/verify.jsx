  ...
  const handleSubmit = async (e) => {
    ...
    if (otp === '') {
      ...
      try {
        ...
        const { data } = await axios.post(`${hostName}/recover/verify`, { email, otp }, config)
          ...
          <Box mt={40} className='form_data1'>
            ...
            <form>
              <div className='form_input_name'>
                ...
                <input type='verify' onChange={(e) => setCodeVerify(e.target.value)} name='verify' id='verify' placeholder='Enter Your OTP ' />
              </div>

              <Button mb={10} color={'white'} onClick={handleSubmit} backgroundColor={'#87b2c4'}>
                Xác thực
              </Button>
            </form>
            <ToastContainer />
