...
const TestItemByJob = ({ jobId, load, setLoad }) => {
  ...
  return <>{tests.length > 0 ? tests.map((test) => <TestItem test={test} />) : <></>}</>
}

const TestItem = ({ test }) => {
  const navigate = useNavigate()
  return (
    <Box borderRadius={10} borderWidth={1} p={5} w={'100%'} m={5}>
      <HStack>
        <VStack>
          ...
          {/* <IconButton aria-label='Search database' icon={<AiOutlineQuestionCircle />} /> */}
        </VStack>
        <Box pl={30} w={'100%'}>
