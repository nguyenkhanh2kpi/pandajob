...
export const CandidateApplyItem = ({ candidate, load, setLoad }) => {
  ...
  useEffect(() => {
    ...
    if (checkedLabelIds.length >= 0 && labels.length > 0) {
      cvService.updateLabel(accessToken, candidate.cvId, JSON.stringify(checkedLabels)).catch((er) => console.log(er))
    }
  }, [checkedLabels])
  ...
  return (
    <>
      <HStack borderRadius={10} boxShadow={'lg'} p={1} bgColor={'white'} w={'100%'}>
        <Flex w={'25%'} spacing='4'>
        ...
        <Text m={0} p={0} w={'25%'} fontStyle={'italic'}>
          Ngày ứng tuyển {candidate.applyDate}
        </Text>
        <Text m={0} p={0} w={'25%'} fontStyle={'italic'}>
          {candidate.jobName}
        </Text>
